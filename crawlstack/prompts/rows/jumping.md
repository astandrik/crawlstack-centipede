Create a single horizontal sprite strip for the Codex app digital pet `crawlstack-polished` in the state `jumping`.
...
Repair attempt 1:
- The previous `jumping` strip failed QA: Visual identity drift against references/canonical-base.png: regenerate the row with only vertical pose changes while preserving the same arched charcoal shell mass, mask scale, trailing embedded masks, palette, dark creepy pixel style, and compact readable silhouette from the canonical Crawlstack identity.
