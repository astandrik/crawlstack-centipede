Create a single horizontal sprite strip for the Codex app digital pet `crawlstack-polished` in the state `waving`.
...
Repair attempt 1:
- The previous `waving` strip failed QA: Visual identity drift against references/canonical-base.png: regenerate the row so the oversized front porcelain mask, teal eyes, charcoal segmented shell, trailing embedded masks, tan markings, tiny stub legs, dark outline weight, gloomy creepy mood, and pixel-art-adjacent finish match the canonical Crawlstack identity exactly.
